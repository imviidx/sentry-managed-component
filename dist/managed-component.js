export default async function (manager, settings) {
    console.log('🎯 [SERVER] Sentry Zaraz Consent Managed Component loading with settings:', Object.keys(settings));
    let currentConsentState = {};
    // Initialize Sentry consent management
    manager.addEventListener('clientcreated', ({ client }) => {
        console.log('[Sentry Zaraz Consent MC] Client created, settings:', settings);
        // Simple hello world to confirm MC is active
        client.execute(`console.log("🚀 Hello from Sentry Zaraz Consent Managed Component!")`);
        client.execute(`console.log("⚙️ MC Settings:", JSON.stringify(${JSON.stringify(settings)}, null, 2))`);
      ...
      (function() {
        ...
        window.addEventListener('consent', function(event) {
          console.log('[Sentry Zaraz Consent MC] Received DOM consent event:', event.detail);
          ...
          if (window.webcm && window.webcm.trigger) {
            ...
            console.log('[Sentry Zaraz Consent MC] WebCM trigger not available, storing consent in window.__consentState');
            window.__consentState = event.detail.payload;

            // Trigger a manual update if managed component is available
            if (window.__sentryZarazConsentMC && window.__sentryZarazConsentMC.updateConsent) {
              window.__sentryZarazConsentMC.updateConsent(event.detail.payload);
            }
          }
        });

        console.log('[Sentry Zaraz Consent MC] DOM consent event listener installed');
      })();
    `);
        client.execute(`
      (function() {
        ...
        if (typeof window.Sentry !== 'undefined' && window.Sentry.getCurrentHub) {
          ...
          if (client && client.getOptions) {
            console.log('[Sentry Zaraz Consent MC] Existing Sentry instance detected - will manage consent settings');
            ...
            window.__sentryZarazConsentMC = {
              initialized: true,
              originalConfig: window.__sentryOriginalConfig,
              updateConsent: function(newConsentState) {
                console.log('[Sentry Zaraz Consent MC] Manual consent update triggered:', newConsentState);
            ...
            console.warn('[Sentry Zaraz Consent MC] Sentry is available but no client found - unable to manage settings');
          }
        } else {
          console.warn('[Sentry Zaraz Consent MC] No existing Sentry instance found. Please initialize Sentry before loading this Managed Component.');
          console.info('[Sentry Zaraz Consent MC] Expected: window.Sentry to be available with getCurrentHub() method');
        }
      })();
    `);
    });
    ...
    manager.addEventListener('clientcreated', ({ client }) => {
        client.execute(`console.log("MC Settings:", ${JSON.stringify(settings)})`);
        client.execute(`
      (function() {
        // Check for stored consent state from DOM events
        if (window.__consentState) {
          console.log('[Sentry Zaraz Consent MC] Found stored consent state:', window.__consentState);
        ...
        window.__updateSentryConsent = function(consentState) {
          console.log('[Sentry Zaraz Consent MC] Direct consent update called:', consentState);
    ...
    function updateSentryConsent(client, consentState) {
        client.execute(`console.log("consentState",${JSON.stringify(consentState)})`);
        client.execute(`
      (function() {
        ...
        window.__updateSentryConsent = function(newConsentState) {
          ...
          if (!window.Sentry || !window.__sentryZarazConsentMC || !window.__sentryZarazConsentMC.initialized) {
            console.warn('[Sentry Zaraz Consent MC] Sentry not properly initialized for consent management');
            return;
          }
          ...
          if (!client) {
            console.warn('[Sentry Zaraz Consent MC] No Sentry client available for configuration update');
            return;
          }

          console.log('[Sentry Zaraz Consent MC] Updating Sentry configuration based on consent:', consent);
          ...
          try {
            ...
            if (!consent.functional) {
              ...
              console.log('[Sentry Zaraz Consent MC] Disabled core tracking features due to functional consent withdrawal');
            } else {
            ...
            if (!consent.analytics) {
              ...
              console.log('[Sentry Zaraz Consent MC] Disabled performance monitoring due to analytics consent withdrawal');
            } else {
              // Use original developer configuration when consent is granted
            ...
            if (!consent.marketing) {
              ...
              console.log('[Sentry Zaraz Consent MC] Disabled session replay due to marketing consent withdrawal');
            } else {
              // Use original developer configuration when consent is granted
            ...
            if (!consent.preferences) {
              ...
              console.log('[Sentry Zaraz Consent MC] Disabled PII collection due to preferences consent withdrawal');
            } else {
              // Use original developer configuration when consent is granted
            ...
            if (!consent.functional) {
              ...
              console.log('[Sentry Zaraz Consent MC] Removed core integrations due to functional consent withdrawal');
            }
            ...
            if (!consent.analytics) {
              ...
              console.log('[Sentry Zaraz Consent MC] Removed analytics integrations due to analytics consent withdrawal');
            }
            ...
            if (!consent.marketing) {
              ...
              console.log('[Sentry Zaraz Consent MC] Removed marketing integrations due to marketing consent withdrawal');
            }
            ...
            if (!consent.preferences) {
              ...
              console.log('[Sentry Zaraz Consent MC] Removed preferences integrations due to preferences consent withdrawal');
            }
            ...
            console.log('[Sentry Zaraz Consent MC] Sentry configuration updated successfully');

          } catch (error) {
            console.error('[Sentry Zaraz Consent MC] Error updating Sentry configuration:', error);
          }
        };
